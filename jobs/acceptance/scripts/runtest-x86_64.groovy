        ...
        stage("Parse variables values from Yaml") {
            steps {
                script {
                    ...
                    env.JOB_OWNER = datas.get(env.JOB_GROUP).get("component").get(params.COMPONENT).get("job_owner")
                    env.XML_FILE = datas.get(env.JOB_GROUP).get("osversion").get(params.OSVERSION).get("xml_file").replaceAll('\\$WORKSPACE', env.WORKSPACE)
                    env.KS_FILE = datas.get(env.JOB_GROUP).get("osversion").get(params.OSVERSION).get("ks_file").replaceAll('\\$WORKSPACE', env.WORKSPACE)
                    env.STAF_CMD = datas.get(env.JOB_GROUP).get("osversion").get(params.OSVERSION).get("staf_cmd")
            ...
            steps {
                script {
                    ...
                    timeout(time: env.TIMEOUT_MONITOR_BEAKER.toInteger(), unit: 'DAYS') {
                        beaker.RunBeakerJob(env.JOB_OWNER, true, env.OUT_FILE, "Acceptance testing(${params.HARDWARE}) - ${params.BREW_NVR}")
                    }
