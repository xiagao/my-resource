...
class UpdateVirtioWin:
    ...
    def check_virtio_win(self):
        ...
        virtio_win_name = self.get_latest_virtio_win_name().strip()
    ...
    def check_released_virtio_win(self):
        ...
        logger.info('Existed released virtio_win_file and its linkname:\n %s' % existed_virtio_win_released)
        virtio_win_name = self.get_released_virtio_win_name().strip()
    ...
    def __get_latest_virtio_win(self):
        ...
        virtio_win_list = []
        for t in self.tag:
            cmd = "brew latest-pkg %s %s" % (t, self.packagename)
            cmd += " | grep virtio | awk '{print $1}'"
            virtio_win_list.append((self.process_cmd(cmd), t))
        
        virtio_list_copy = virtio_win_list[::]

        print "***************virtio_win_list[0][0] is %s" % virtio_win_list[0][0]
        print "***************virtio_win_list[1][0] is %s" % virtio_win_list[1][0]
        if virtio_win_list[0][0] == virtio_win_list[1][0]:
            ...
            for i, j in virtio_win_list:
                if '_' in i:
                    ...
                    virtio_list_copy.remove((i, j))           
            for i, j in virtio_list_copy:
                return i, j     
    ...
    def get_released_virtio_win_name(self):
        ...
        logger.info('The released virtio_win_name is: %s' % released_virtio_win_name)
    ...
    def get_released_virtio_win_tag(self):
        ...
        logger.info('The released virtio_win_tag is: %s' % released_virtio_win_tag)
    ...
    def download_released(self):
        ...
        cmd = "cd %s;brew download-build --arch=noarch --latestfrom=%s %s"
        cmd = cmd % (self.workdir, tagname_released, self.packagename)
        logger.info('Start to Download virtio_win rpm package: %s' % self.packagename)
        self.process_cmd(cmd)
    ...
    def decompress_rpm_package(self):
        ...
        :return: None
        """
        package = "%s.noarch.rpm" % self.get_latest_virtio_win_name().strip()
    ...
    def decompress_released_rpm_package(self):
        package = "%s.noarch.rpm" % self.get_released_virtio_win_name().strip()
        cmd = "cd %s;rpm2cpio %s | cpio -dimv" % (self.workdir, package)
        logger.info('Start to execute %s' % cmd)
        logger.info('Start to decompress: %s' % package)
        self.process_cmd(cmd)
    ...
    def copy_released_to_nfs(self):
        ...
        :return: None
        """
        logger.info('Start to copy released packages to nfs server: %s' % self.nfsserver)
    ...
    def make_link_released(self):
        ...
        def __create(imagetype):
            virtio_list = self.__get_released_virtio_win_name_list()
            print "1. ***********virtio_list is %s" % virtio_list
            prefix = 'virtio-win'
            for virtio_win_name in virtio_list:
                os.chdir(self.nfspath)
                if virtio_win_name.endswith(imagetype):
                    pattern = '%s(_\d)?' % self.virtio_win_version
                    linkname = re.search(pattern, virtio_win_name)
                    if linkname is not None:
                        link = linkname.group(0)
                        link = prefix + '_' + imagetype + '.' + link
                        print "2. ***********link is %s" % link
                        self.released_virtio_win_name_list.append(os.path.basename(virtio_win_name))
                        self.__create_soft_link(os.path.basename(virtio_win_name), link)
        for image in image_type:
            __create(image)
    ...
    def update_virtio_win(self):
        ...
        if not self.check_virtio_win():
            ...
            logger.warn('%s exists, will not recreate it.' % self.get_latest_virtio_win_name())

    def update_released_virtio_win(self):
        """
        update the latest virtio-win version for released rhel,like 7.4/6.9
        """
        ...
        if not self.check_released_virtio_win():
            ...
            logger.warn('%s exists, will not recreate it.' % self.get_released_virtio_win_name())
...
def main():
    ...
    # rhel7.5 will use the virito_win version belonging to rhel7.4
    # this is because virtio_win package comes out slowly
    # so always keep the latest one
    if rhel:
        for osname in rhel:
            logger.info("rhel version is %s" % osname)
            tag = cfg.get(osname, 'tagname').split()
            logger.info("virtio_win tag is %s" % tag)
            if osname.find('RHEL8') != -1:
                virtio_win_version = 'el8'
    ...
    # for example:
    # before virtio_win of rhel7.5 comes out ,we will use virtio_win of rhel7.4
    # but if virtio_win of rhel7.5 comes out ,then we will add the virtio_win of rhel7.4
    # to the released_list in cfg file. and then update the os_list to rhel7.5
    if released_rhel:
        for releasedosname in released_rhel:
            logger.info("released rhel version is %s" % releasedosname)
            tag_released =cfg.get(releasedosname, 'tagname_released')
            logger.info('tag_released %s' % tag_released)
            logger.info("released virtio_win tag is %s" % tag_released)
            if osname.find('RHEL8') != -1:
                virtio_win_version = 'el8'
                logger.info('virtio_win_version is %s' % virtio_win_version)
