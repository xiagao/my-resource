<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:variable name="distro_is_fedora">
    <xsl:choose>
      <xsl:when test="contains(/submit/recipe/machine/@distro, 'Fedora')">
        <xsl:value-of select="'yes'"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="'no'"/>
    ...
    <hostRequires>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <and>
            <xsl:choose>
              <xsl:when test="string-length(@sys_pool) &gt; 0">
                <xsl:choose>
                  <xsl:when test="starts-with(@sys_pool, '!')">
                    <pool op="!=">
                      <xsl:attribute name="value"><xsl:value-of select="substring(@sys_pool, 2)"/></xsl:attribute>
                    </pool>
                  </xsl:when>
                  <xsl:otherwise>
                    <pool op="=">
                      <xsl:attribute name="value"><xsl:value-of select="@sys_pool"/></xsl:attribute>
                    </pool>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:when>
            ...
            <xsl:for-each select="../host_filters/filter">
              <xsl:choose>
                ...
                <xsl:when test=". = 'min-mem-32g'">
                  <memory op=">=" value="32768"/>
                </xsl:when>
        ...
        <distro_variant op="=">
          <xsl:attribute name="value">
            <xsl:choose>
              ...
              <xsl:when test="$distro_is_fedora = 'yes'">
                <xsl:text>Server</xsl:text>
              </xsl:when>
              <xsl:otherwise>
