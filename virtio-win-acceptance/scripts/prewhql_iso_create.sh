...
then
    ...
    then
        # mount 10.66.8.170:/home/nfs $NFS_MNT
...
then
    ...
    # qemu-ga-win-101.0.0-5.el7ev.noarch
    QEMU_GA_MAIN_VER_RPM=$(rpm -q qemu-ga-win | sed -n 's/.*-\([0-9]\+\.[0-9]\+\.[0-9]\+\)-.*/\1/p')
    QEMU_GA_SUB_VER_RPM=$(rpm -q qemu-ga-win | sed -n 's/.*-\([0-9]\+\.[a-z]\+[0-9]\+[a-z]\+\)\..*/\1/p')
...
set -e
# Copy iso file,from 177 version, only have win8+ os
echo "+===========================================+"
...
#iso dirver path
x86_os="w8 w8.1 w10"
x64_os="2k12 2k12R2 w8 w8.1 w10 2k16 2k19"
ARCH="amd64 x86"
...
#vfd dirver path
ARCH_VFD="amd64 x86 servers_amd64"
amd64_os_vfd="Win8 Win8.1 Win10"
servers_amd64_os_vfd="Win2012 Win2012R2 Win2016 Win2019"
x86_os_vfd="Win8 Win8.1 Win10"
...
do
    ...
    do
        ...
        do
            if [ $dst_os == "Win8" -o $dst_os == "Win8.1" -o $dst_os == "Win2012" -o $dst_os == "Win2012R2" ]
            then
                src_os="Win8"
