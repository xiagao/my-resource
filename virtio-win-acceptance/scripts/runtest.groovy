        ...
        stage("Parse configurations for the tests") {
            steps {
                script {
                    ...
                    kar_cmd = confs.get('kar_cmd_list').get(kar_cmd_index).get('cmd')
                    kar_cmd += " --customsparams=\"cdrom_virtio = isos/windows/${env.NVR}.iso\""
                    if (env.PKG_NAME.contains('prewhql')) {
                        accep_loop_name = 'virtio_win_prewhql_acceptance'
                    } else {
                        accep_loop_name = 'virtio_win_acceptance'
                    }
                    env.KAR_CMD = kar_cmd.replaceAll('\\$\\{\\{LOOP_NAME\\}\\}', accep_loop_name)
                    ...
                    env.ENV_OPTIONS = "--stable --verbose"
